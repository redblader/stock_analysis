...
class URLData:
    ...
    def __init__(self, proxy_flag=False):
        ...
        self._elem_list = ['name', 'open', 'pre_close', 'curr_price', 'high', 'low', 'buy_1', 'sell_1', 'volume', 'amount', 
                           'buy_v1', 'buy_p1', 'buy_v2', 'buy_p2', 'buy_v3', 'buy_p3', 'buy_v4', 'buy_p4', 'buy_v5', 'buy_p5',
                           'sell_v1', 'sell_p1', 'sell_v2', 'sell_p2', 'sell_v3', 'sell_p3', 'sell_v4', 'sell_p4', 'sell_v5', 'sell_p5',
                           'curr_date', 'curr_time']
        self._storage = dict()
    ...
    def get_data(self, stock_name):
        if stock_name in self._storage:
            for i in range(3):
                try:
                    resp = urllib2.urlopen('http://hq.sinajs.cn/list=' + stock_name, timeout=5)
                    break
                except:
                    if i == 2:
                        return
